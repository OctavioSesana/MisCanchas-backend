...
async function findAll(req: Request, res: Response) {
  try {
    ...
    res
      .status(200)
      .json({ message: "found all types", data: reservas_articulos });
  } catch (error: any) {
...
async function add(req: Request, res: Response) {
  try {
    const reserva_articulo = em.create(
      ReservaArticulo,
      req.body.sanitizedInput
    );
    await em.flush();
...
async function update(req: Request, res: Response) {
  try {
    const id = Number.parseInt(req.params.id);
    const reserva_articuloToUpdate = await em.findOneOrFail(ReservaArticulo, {
      id,
    });
