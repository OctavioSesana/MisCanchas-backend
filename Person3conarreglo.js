...
contCliente = 0;
while (opcion === "S") {
    p = prompt("Carga datos Cliente - Ingrese su DNI:");
    if (isNaN(p)) {
        console.log("El DNI ingresado no es valido. Por favor, vuelva a intentarlo.");
        continue;
    } 
    // Validacion del dni con el arreglo version 1.0
    let op = 0;
    for (let i = 0; i < p.length; i++){
      if (p === per[i]){
        op = 1;
        console.log ("El dni ingesado ya se encuentra reistrado, por favor ingrese otro");
        break;
      }
